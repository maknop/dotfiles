...
FISH_FILES := $(shell find .config/fish -name '*.fish')
NVIM_CONFIG := .config/nvim
SHELLCHECK := shellcheck
FISH := fish
LUACHECK := luacheck
LUA_DIRS := .config/nvim/lua
...
.PHONY: help lint shellcheck fish-lint lua-lint check-shellcheck check-fish check-luacheck install test clean all
...
help:
	...
	@echo "  $(GREEN)lint$(NC)       - Run all linting checks (shellcheck + fish + lua)"
	@echo "  $(GREEN)shellcheck$(NC) - Run shellcheck on all shell scripts"
	@echo "  $(GREEN)fish-lint$(NC)  - Check fish syntax on all .fish files"
	@echo "  $(GREEN)lua-lint$(NC)   - Run luacheck on Neovim Lua config"
	...
	@echo "  make fish-lint     # Run only fish syntax check"
	@echo "  make lua-lint      # Run only luacheck"
...
check-fish:
	@if ! command -v $(FISH) >/dev/null 2>&1; then \
		echo "$(RED)Error: fish is not installed$(NC)"; \
		echo "Install it with: brew install fish  (or your distro's package manager)"; \
		exit 1; \
	fi

# Check fish syntax on all .fish files
fish-lint: check-fish
	@echo "$(BLUE)Running fish syntax check...$(NC)"
	@failed=0; \
	for file in $(FISH_FILES); do \
		printf "Checking %-50s " "$$file:"; \
		if $(FISH) --no-execute "$$file" >/dev/null 2>&1; then \
			echo "$(GREEN)✓ PASS$(NC)"; \
		else \
			echo "$(RED)✗ FAIL$(NC)"; \
	...
	if [ $$failed -eq 0 ]; then \
		echo "$(GREEN)All fish files passed syntax check!$(NC)"; \
	else \
		echo "$(RED)Some fish files failed syntax check.$(NC)"; \
		exit 1; \
	fi
...
check-luacheck:
	@if ! command -v $(LUACHECK) >/dev/null 2>&1; then \
		echo "$(RED)Error: luacheck is not installed$(NC)"; \
		echo "Install it with: luarocks install luacheck"; \
		exit 1; \
	fi

# Run luacheck on Neovim Lua config
lua-lint: check-luacheck
	@echo "$(BLUE)Running luacheck on Lua files...$(NC)"
	@$(LUACHECK) $(LUA_DIRS)

# Run all linting checks
lint: check-shellcheck check-fish check-luacheck shellcheck fish-lint lua-lint
